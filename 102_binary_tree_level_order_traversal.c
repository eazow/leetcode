...
int **levelOrderBottom(struct TreeNode *root, int **columnSizes, int *returnSize) {
    ...
    Queue *vals_queue = queue_create();
    Queue *nodes_count_queue = queue_create();
    int nodes_count = 0;
    int i = 0;
    ...
    while(!queue_empty(queue)) {
        ...
        queue_push(nodes_count_queue, (struct TreeNode *)(intptr_t)nodes_count);
        ...
        for(i = 0; i < nodes_count; i++) {
            ...
            queue_push(vals_queue, (struct TreeNode *)(intptr_t)tree_node->val);
        }
    }

    int j = 0;
    *returnSize = queue_size(nodes_count_queue);
    *columnSizes = (int *)malloc(sizeof(int) * (*returnSize));
    int **return_arrays = (int **)malloc(sizeof(int *) * (*returnSize));


    while(!queue_empty(nodes_count_queue)) {
        nodes_count = (int)(intptr_t)queue_pop(nodes_count_queue);
        *(*columnSizes+j) =  nodes_count;
        int *nums = (int *)malloc(sizeof(int) * nodes_count);
        for(i = 0; i < nodes_count; i++) {
            nums[i] = (int)(intptr_t)queue_pop(vals_queue);
        }
        return_arrays[j] = nums;
        j++;
    }

    queue_destroy(nodes_count_queue);
    queue_destroy(vals_queue);
    queue_destroy(queue);
